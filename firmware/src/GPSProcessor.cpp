...
void GPSProcessor::processGPSData() {
    ...
    if (statusChanged) {
        ...
        if (timeIsValid) {
            Serial.printf("GPS Time Valid - Satellites: %d | HDOP: %.1f | Signal: %s\n", 
                         satelliteCount, hdop, signalStrength);
        } else {
            Serial.printf("GPS acquiring... Satellites: %d | Signal: %s\n", 
                         satelliteCount, signalStrength);
        }
